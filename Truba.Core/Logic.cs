...
{
    public static T GetValue<T>(Pipe<T> pipe) => 
        pipe.Value;

    public static Pipe<T> GetPipeV<T>(T value) => 
        new(value);

    public static Pipe<TResult> GetPipeF<TArgument, TResult>(Pipe<TArgument> argumentPipe, Func<TArgument, TResult> func) =>
        GetPipeV(func(GetValue(argumentPipe)));
    ...
    public static Task<T> GetValue<T>(AsyncPipe<T> asyncPipe) => 
        asyncPipe.Value;

    public static AsyncPipe<T> GetPipeVAsync<T>(Task<T> value) => 
        new(value);
    ...
    public static AsyncPipe<TResult> GetPipeFAsync<TArgument, TResult>(AsyncPipe<TArgument> asyncPipe, Func<TArgument, Task<TResult>> funcAsync) =>
        GetPipeVAsync(BindAsync(GetValue(asyncPipe), funcAsync));
